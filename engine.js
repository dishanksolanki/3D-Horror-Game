...
washroom1.js / room11.js / room12.js / room13.js files,
and BEFORE main.js.
============================================================ */
...
ctx.translate(cx,cy);
ctx.beginPath();
const points = 13 + Math.floor(Math.random()*9);
const radiusScale = 0.22 + Math.random()*0.14;
...
radials.push(a);
if(Math.random()<tornAmount) continue;
ctx.beginPath(); ctx.moveTo(cx,cy);
ctx.lineTo(cx+Math.cos(a)*260, cy+Math.sin(a)*260);
ctx.stroke();
}
...
// click to open/close whichever almirah drawer / gate the player is looking at
...
if(o.isGate && o.gateRef && o.gateRef.isOpen) continue; // open gates don't block movement
...
if(bulbLight){
const flicker = Math.random() < 0.02 ? Math.random()*0.4 : 0;
...
// gate doors (room11's north gate, etc.) swinging open/closed
gates.forEach(g=>{
