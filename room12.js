/* ============================================================
   ...
   be loaded first, AND requires room11.js to already cut a
   doorway gap into room 11's east wall — without that this room
   is unreachable.
============================================================ */
...
let room12Light, corridor12Light;
...
function buildRoom12(){
  ...
  // floor/ceiling: X-extent and Z-extent both use ROOM12_W, since
  // that's the same constant that defines the room's real east/west
  // wall positions (ROOM12_EAST_X/WEST_X) AND the room's real
  // north/south wall spacing below - keeps every wall's corner
  // meeting cleanly instead of overshooting or falling short.
  const floor = new THREE.Mesh(new THREE.PlaneGeometry(ROOM12_W, ROOM12_W), floorMat);
  floor.rotation.x = -Math.PI/2;
  floor.position.set(centerX, 0, cz);
  floor.receiveShadow = true;
  scene.add(floor);

  const ceil = new THREE.Mesh(new THREE.PlaneGeometry(ROOM12_W, ROOM12_W), ceilMat);
  ceil.rotation.x = Math.PI/2;
  ...
  const southWall = new THREE.Mesh(new THREE.PlaneGeometry(ROOM12_W, ROOM12_H), wallMat.clone());
  ...
  const trimS = new THREE.Mesh(new THREE.BoxGeometry(ROOM12_W,0.15,0.1), trimMat);
