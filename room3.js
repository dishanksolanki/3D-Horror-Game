...
function buildRoom3(){
  ...
  // carries the doorway; other three walls are solid dead-ends for now
  // (open one later the same way to keep expanding).
  ...
  // east (far) wall - solid dead-end
  const eastWall = new THREE.Mesh(new THREE.PlaneGeometry(ROOM3_W, ROOM3_H), wallMat);
  eastWall.position.set(ROOM3_EAST_X, ROOM3_H/2, cz);
  ...
  // baseboard trim
  const trimMat = new THREE.MeshStandardMaterial({color:0x120c08, roughness:1});
  const trimE = new THREE.Mesh(new THREE.BoxGeometry(0.1,0.15,ROOM3_W), trimMat);
  trimE.position.set(ROOM3_EAST_X,0.08,cz); scene.add(trimE);
